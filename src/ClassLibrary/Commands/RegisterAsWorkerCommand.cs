...
/// no estan definidas en el contrato del tipo (abierto a la extensión), pero sin modificar lo que ya estaba hecho (cerrado a la modificación).
/// </summary>
public class RegisterAsWorkerCommand : ICommand
{
    /// <summary>
    ...
    /// </summary>
    public RegisterAsWorkerCommand()
    {
        this.Name = "/registerasworker";
    ...
    /// </summary>
    /// <value></value>
    public RegisterAsWorkerState State { get; private set; } = RegisterAsWorkerState.NoStarted;
    ...
    /// </summary>
    /// <returns></returns>
    private Dictionary<long, RegisterAsWorkerState> stateForUser = new Dictionary<long, RegisterAsWorkerState>();

    /// <summary>
    /// Los datos  para un usuario que va obteniendo el comando en los diferentes estados.
    /// </summary>
    private Dictionary<long, WorkerData> data = new Dictionary<long, WorkerData>();
    
    private WorkerData dataOfWorker;
    ...
    {
        stateForUser[userID] = RegisterAsWorkerState.NoStarted;
        this.State = RegisterAsWorkerState.NoStarted;
    ...
    public void RestoreData(long userID)
    {
        ...
        else
        {
            this.State = RegisterAsWorkerState.NoStarted;
            stateForUser[userID] = RegisterAsWorkerState.NoStarted;
            dataOfWorker = new WorkerData();
            data[userID] = dataOfWorker;
            this.InProccess = true;
        }
    ...
    {
        RestoreData(userID);
        if (this.State == RegisterAsWorkerState.NoStarted)
        {
         dataOfWorker.Response = "Ahora pasaremos a pedir los datos para crear un trabajador. \nIngrese el nombre del trabajador";
         stateForUser[userID] = RegisterAsWorkerState.Started;
        }
        else if (this.State == RegisterAsWorkerState.Started)
        {
            //Validamos que el nombre este correcto
            if (!string.IsNullOrWhiteSpace(inputData))
            {
                dataOfWorker.Name = inputData;
                stateForUser[userID] = RegisterAsWorkerState.LastNamePromt;
                dataOfWorker.Response = "Ingrese su apellido.";
            }
            else
            {
                dataOfWorker.Response = "Su nombre no puede estar vacío.";
            }
        }
        else if (this.State == RegisterAsWorkerState.LastNamePromt)
        {
            //Validamos que el apellido este correcto
            if (!string.IsNullOrWhiteSpace(inputData))
            {
                dataOfWorker.LastName = inputData;
                stateForUser[userID]= RegisterAsWorkerState.PhonePromt;
                dataOfWorker.Response = "Ingrese su número de celular.";
            }
            else
            {
                dataOfWorker.Response = "Su apellido no puede estar vacío.";
            }
        }
        else if (this.State == RegisterAsWorkerState.PhonePromt)
        {
            ...
            if (inputData.Length == 9)
            {
                if(match.Success)
                {
                    dataOfWorker.Phone = inputData;
                    stateForUser[userID] = RegisterAsWorkerState.AddressPromt;
                    dataOfWorker.Response = "Ingrese su dirección por favor.";
                }
                else
                {
                    dataOfWorker.Response = "El celular que ingreso no tiene un formato valido. El formato valido es: 09[1-9]xxxxxx. Cambiando las x por los números correspondientes";
                }
            }
            else
            {
                dataOfWorker.Response = "Su número de celular debe tener 9 dígitos.";
            }
        }
        else if (this.State == RegisterAsWorkerState.AddressPromt)
        {
            if (!string.IsNullOrWhiteSpace(inputData))
            {
                dataOfWorker.Address = inputData;
                // Ahora revisa si la ubicación es valida
                this.dataOfWorker.Result = this.finder.GetLocation(this.dataOfWorker.Address);
                ...
                if (this.dataOfWorker.Result.Found)
                {
                    // Si encuentra la dirección pasa nuevamente al estado Initial
                    Database.Instance.AddWorker(userID, dataOfWorker.Name, dataOfWorker.LastName, dataOfWorker.Phone, dataOfWorker.FormattedAddress, dataOfWorker.Result.Latitude, dataOfWorker.Result.Longitude);
                    // Finalizamos el comando usando este metódo
                    Cancel(userID);
                    // Esto me parece que debería ir con Check
                    if(Database.Instance.ExistWorker(userID))
                    {
                        dataOfWorker.Response = $"Se hizo correctamente el registro, ahora eres un trabajador. \nNombre: {dataOfWorker.Name} {dataOfWorker.LastName} \nTeléfono: {dataOfWorker.Phone} \nUbicación: {dataOfWorker.FormattedAddress}";
                    }
                    else
                    {
                        dataOfWorker.Response = "No se pudo completar la tarea por un error del sistema";
                    }
                }
                else
                {
                    // Si no encuentra la dirección se la pide de nuevo y queda en el estado AddressPrompt
                    dataOfWorker.Response = "No encuentro la dirección. Decime qué dirección querés buscar por favor";
                }
            }
            else
            {
                dataOfWorker.Response = "Su ubicación debe ser una palabra o palabras válidas.";
            }
        }
        else
        {
            dataOfWorker.Response = "Se canceló la ejecución";
        }
        data[userID] = dataOfWorker;
        return dataOfWorker.Response;
    }

    /// <summary>
    /// Determina los estados que tiene el comando.
    /// </summary>
     public enum RegisterAsWorkerState
    {
